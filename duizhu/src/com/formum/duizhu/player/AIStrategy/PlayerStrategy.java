...
public class PlayerStrategy implements IPlayerStrategy {
	...
	private static volatile PlayerStrategy INSTANCE = null;
	private IUtil tools = Util.getInstance();
	private IPowerCardsBag robotPowerCardsBag = RobotPowerCardsBag
			.getInstance(RobotPlayer.getInstance(AIPlay.getInstance()));
	...
	public static PlayerStrategy getInstance() {
		if (PlayerStrategy.INSTANCE == null) {
			synchronized (PlayerStrategy.class) {
				if (PlayerStrategy.INSTANCE == null) {
	...
	private int initStrategy(List<ICard> cardsInHand, int result) {
		...
		if (size13) {
			...
			boolean hasParityTrumpCards = ((robotPowerCardsBag
					.getOtherTrumpCards().size()) >= 4);// 平均主
			boolean hasLongTrumpCards = (robotPowerCardsBag
					.getOtherTrumpCards().size() >= 7);// 主长趟（21张主，每人平均5.5张不到，7张以上算多了）
			...
			// 在手弱牌w数 _ 分数
			boolean w1_0 = robotPowerCardsBag
					.allNorStraightDangerScoreCardsSum() == 0;
			boolean w1_5 = robotPowerCardsBag
					.allNorStraightDangerScoreCardsSum() == 5;
			boolean w1_10 = robotPowerCardsBag
					.allNorStraightDangerScoreCardsSum() == 10;
			boolean w2_5 = robotPowerCardsBag
					.allNorStraightDangerScoreCardsSum() == 5;
			...
			// 任意一种都不破
			boolean safescore = score1_5 || score1_10 || score1_15 || score1_20
					|| score2_25;

			// 打光策略
			boolean target_win_both_0score_joker = roundWinComboJoker
					&& score0_0;
			boolean target_win_both_0score = roundWinCombo && score0_0;
			// 打成策略
			boolean target_win_both_fewscore_joker = roundWinComboJoker
					&& safescore;
	...
	@Override
	public List<ICard> suggestCardsAsForeHand(int target,
			IPowerCardsBag powerCardsBag) {
		
		List<ICard> suggestCards = new ArrayList<ICard>();
		// 先手，根据策略号，经过规则检测
		boolean strategy1001 = (target == IBankerStrategy.TARGET_WIN_BOTH_0SCORE_JOKER);// 有王必扣可能破
		boolean strategy1000 = (target == IBankerStrategy.TARGET_WIN_BOTH_0SCORE);// 长主必扣可能破
		boolean strategy2001 = (target == IBankerStrategy.TARGET_WIN_BOTH_FEWSCORE_JOKER);// 有王必扣可能破
		boolean strategy2000 = (target == IBankerStrategy.TARGET_WIN_BOTH_FEWSCORE);// 长主必扣可能破
		boolean strategy3001 = (target == IBankerStrategy.TARGET_WIN_ROUND_JOKER);// 有王可能扣底
		boolean strategy3000 = (target == IBankerStrategy.TARGET_WIN_ROUND);// 长主可能扣底
		boolean strategy4000 = (target == IBankerStrategy.TARGET_WIN_SCORES);// 主弱可能打破
		if (strategy1001) {
	...
	private void suggest_4000(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 主弱保分
		...
		if (cards4) {
			...
			// 有甩先出
			boolean heartStraight = !powerCardsBag.getStraightHeartCards()
					.isEmpty();
			boolean clubStraight = !powerCardsBag.getStraightClubCards()
					.isEmpty();
			boolean diamondStraight = !powerCardsBag.getStraightDiamondCards()
					.isEmpty();
			boolean spadeStraight = !powerCardsBag.getStraightSpadeCards()
					.isEmpty();
			// 暂时按此顺序（有牌就行）出
			if (spadeStraight) {
				...
				if (hasComplex) {
					...
					if (hasScore_Nor) {
						...
						for (ICard iCard : score_norscoreCards) {
							boolean hasNoScore = (iCard.getPoint() != IGameRule.CARD_5_EQUAL_SCORE
									&& iCard.getPoint() != IGameRule.CARD_10_EQUAL_SCORE && iCard
									.getPoint() != IGameRule.CARD_13_EQUAL_SCORE);
							if (hasNoScore) {
					...
					// 再到主牌,先大再小，控制牌权
					boolean cardPoint2 = !powerCardsBag.getPoint2TrumpCards()
							.isEmpty();// 看有没有牌2
					boolean cardClassPoint = !powerCardsBag
							.getClassPointTrumpCards().isEmpty();// 看有没有将牌
					boolean joker2 = !powerCardsBag.getJoker2TrumpCards()
							.isEmpty();// 有没有小王

					// 杂主要将有分主牌排除掉
					List<ICard> otherTrumpsCards = powerCardsBag
							.getOtherTrumpCards();
					List<ICard> scoreTrumpCards = powerCardsBag
							.getScoreTrumpCards();
					otherTrumpsCards.removeAll(scoreTrumpCards);

					boolean othertrumps = !otherTrumpsCards.isEmpty();// 有没有去分后的杂主
					boolean scoreTrumps = !powerCardsBag.getScoreTrumpCards()
							.isEmpty();// 分主牌
					if (cardPoint2) {
						suggestCards.add(powerCardsBag.getPoint2TrumpCards()
								.get(0));
					} else if (cardClassPoint) {
						suggestCards.add(powerCardsBag
								.getClassPointTrumpCards().get(0));
					} else if (joker2) {
						suggestCards.add(powerCardsBag.getJoker2TrumpCards()
								.get(0));
					} else if (othertrumps) {
						// 杂主排序，出大牌
						List<ICard> otherTrumps = powerCardsBag
								.getOtherTrumpCards();
						// 先找ace
						...
						// 有分主，先出小分牌
						suggestCards.add(powerCardsBag.getScoreTrumpCards()
								.get(0));
					} else {
	...
	private void suggest_3000(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 主长，必须抢掉主-策略与1000相当
		boolean cards4 = !powerCardsBag.getCards4().isEmpty();// 看翁牌有没有
		boolean cardPoint2 = !powerCardsBag.getPoint2TrumpCards().isEmpty();// 看有没有牌2
		boolean cardClassPoint = !powerCardsBag.getClassPointTrumpCards()
				.isEmpty();// 看有没有将牌
		...
		if (opponentTrumps) {
			if (cards4) {
				...
				suggestCards
						.add(powerCardsBag.getClassPointTrumpCards().get(0));
			} else if (joker2) {
			...
			if (spadeStraight) {
				...
				if (hasComplex) {
					...
					// 再没副就出主了
					boolean hasTrumpToo = !powerCardsBag.getOtherTrumpCards()
							.isEmpty();
					if (hasTrumpToo) {
						suggestCards.add(powerCardsBag.getOtherTrumpCards()
								.get(0));
					} else if (scoreTrumps) {
						suggestCards.add(powerCardsBag.getScoreTrumpCards()
								.get(0));
	...
	private void suggest_3001(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 保王，副牌抢控制权--清副牌
		// 有翁出翁
		boolean cards4 = !powerCardsBag.getCards4().isEmpty();// 看翁牌有没有
	...
	private void suggest_2000(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 此策略关键掉主--与1001的策略相当
		boolean cards4 = !powerCardsBag.getCards4().isEmpty();// 看翁牌有没有
		boolean cardPoint2 = !powerCardsBag.getPoint2TrumpCards().isEmpty();// 看有没有牌2
	...
	private void suggest_2001(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 此策略关键保大王同时清分要果断
		boolean cards4 = !powerCardsBag.getCards4().isEmpty();// 看翁牌有没有
		if (cards4) {
	...
	private void suggest_1000(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
	...
	private void suggest_1001(IPowerCardsBag powerCardsBag,
			List<ICard> suggestCards) {
		// 应当强行消耗对方的主牌，令其无法对自己的副牌构成威胁
		boolean cards4 = !powerCardsBag.getCards4().isEmpty();// 看翁牌有没有
		boolean cardPoint2 = !powerCardsBag.getPoint2TrumpCards().isEmpty();// 看有没有牌2
 ...
 */
	private void makeSureSuggestCard(List<ICard> suggestCards) {
		...
		if (suggestCardsZero) {
			...
			System.out.println("@*PlayerStrategy makeSureSuggestCard()--suggestCard-->"+suggestCards.get(0).getSuit()+suggestCards.get(0).getPoint());
		    }
		}
	}
	...
	public List<ICard> suggestCardsAsAfterHand(IPowerCardsBag powerCardsBag, List<ICard> opponentCards) {
		...
		// 能不能管牌
		boolean canSuppress = this.canSuppressOpponentCards(powerCardsBag,
				opponentInfo, opponentCards);
		// 应该管否
		boolean shouldSuppress = this.shouldSuppressOpponentCards(
				powerCardsBag, opponentInfo, opponentCards);

		if (canSuppress && shouldSuppress) {

			// 给出管牌建议----最小代价，尽可能带分跑
			suggestCards.addAll(this.makeSuppressSuggestion(powerCardsBag,
					opponentInfo,opponentCards));
		} else {
			// 给出随牌建议----原则是‘最小代价’（小点牌，最好不加入分）
			suggestCards.addAll(this.makeFollowSuggestion(powerCardsBag,
					opponentInfo,opponentCards));
		}
	...
	private List<ICard> makeFollowSuggestion(	IPowerCardsBag powerCardsBag, Map<String, Integer> opponentInfo,List<ICard> opponentCards) {
		...
		switch (type) {
		case 1:
			System.out.println("@PlayerStrategy  makeFollowSuggestion 翁");	
			result.addAll(powerCardsBag.followCards4(opponentCards));
			System.out.println("@PlayerStrategy  makeFollowSuggestion 翁"+result.size());
			break;
		case 2:
		    System.out.println("@PlayerStrategy  makeFollowSuggestion 甩");	
			result.addAll(powerCardsBag.followStraightCards(opponentCards));
			System.out.println("@PlayerStrategy  makeFollowSuggestion 甩"+result.size());
			break;
		case 3:
		    
		    System.out.println("@PlayerStrategy  makeFollowSuggestion 主");	
			result.addAll(powerCardsBag.followTrumpCard(opponentCards));
			System.out.println("@PlayerStrategy  makeFollowSuggestion 主"+result.size());
			break;
		case 4:
			System.out.println("@PlayerStrategy  makeFollowSuggestion  副");	
			result.addAll(powerCardsBag.followNorTrumpCard(opponentCards));
			System.out.println("@PlayerStrategy  makeFollowSuggestion 副"+result.size());
			break;
	...
	private boolean shouldSuppressOpponentCards(IPowerCardsBag powerCardsBag,
			Map<String, Integer> opponentInfo, List<ICard> opponentCards) {
...
private boolean thereIsOddSameSuitScoreCard(List<ICard> opponentCards) {
	...
	private boolean canSuppressOpponentCards(IPowerCardsBag powerCardsBag,
			Map<String, Integer> opponentInfo, List<ICard> opponentCards) {
		boolean result = false;
		// 看信息返回类型，与手牌比对，用规则比较
		...
		case 1:
			...
			if (hasMyCards4) {
				...
				Player player = gameRule.whoIsRoundWinner(opponentCards,
						powerCardsBag.getCards4());
		...
		case 2:
			...
			String suit = opponentCards.get(0).getSuit();
			boolean noTheSuit = tools.seperateNoTrumpSameSuitCards(
					robot.actions().getSelfCardDeck(), suit).isEmpty();
			if (noTheSuit) {
				// 看主牌数量>=对手牌数
				int trumpSize = tools.seperateTrumpsFromCards(
						robot.actions().getSelfCardDeck(), recorder).size();
		...
		case 3:
			// 看主牌有无，有的话，能否压制
			boolean hasTrump = !tools.seperateTrumpsFromCards(
					robot.actions().getSelfCardDeck(), recorder).isEmpty();
			if (hasTrump) {
				List<ICard> myTrumpCards = tools.seperateTrumpsFromCards(robot
						.actions().getSelfCardDeck(), recorder);
		...
		case 4:
			...
			boolean noSuit = tools.seperateNoTrumpSameSuitCards(
					robot.actions().getSelfCardDeck(), thesuit).isEmpty();
			if (noSuit) {
				// 看主牌数量>=对手牌数
				int trumpSize = tools.seperateTrumpsFromCards(
						robot.actions().getSelfCardDeck(), recorder).size();
				...
				// 有同花牌
				List<ICard> sameSuitCards = tools.seperateNoTrumpSameSuitCards(
						robot.actions().getSelfCardDeck(), thesuit);
				List<ICard> list = new ArrayList<ICard>();
				IGameRule gameRule = GameRule.getInstance();
	...
	private Map<String, Integer> analysisInfo(List<ICard> opponentCards) {
		...
		if (isCards4) {
		    info.put("type", 1);
		    System.out.println("@PlayerStrategy analysisInfo(List<ICard> opponentCards)--isCards4-->"+isCards4);
		}else{
		    // 主牌
		    boolean isTrumpCard = tools.isAllTrumpCards(opponentCards) && opponentCards.size() == 1;
		    if (isTrumpCard) {
			info.put("type", 3);
			System.out.println("@PlayerStrategy analysisInfo(List<ICard> opponentCards)--isTrumpCard-->"+isTrumpCard);
		    }else{
			    // 甩副
			    ...
			    if (isStraightCards) {
				info.put("type", 2);
				System.out.println("@PlayerStrategy analysisInfo(List<ICard> opponentCards)--isStraightCards-->"+isStraightCards);
			    }else{
			    // 副，只有一张，选剩的
				info.put("type", 4);
				System.out.println("@PlayerStrategy analysisInfo(List<ICard> opponentCards)--isNonTrumpCard-->选剩的");
			    }
